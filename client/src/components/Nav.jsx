...
export default function Nav() {
  ...
  return (
    <header ref={headerRef} className="sticky top-0 z-40 bg-ink-950/85 backdrop-blur-lg border-b border-ink-700/60 safe-top">
      {/* The wordmark + four buttons must fit a 320px viewport: the logo steps
          down and the right cluster tightens below sm, or the page scrolls sideways. */}
      <div className="max-w-7xl mx-auto px-3 sm:px-4 h-16 flex items-center gap-2 sm:gap-3">
        <Link to="/discover" className="shrink-0 min-w-0"><Logo className="h-10 sm:h-[58px]" /></Link>
        ...
        <div className="flex items-center gap-1 sm:gap-2 ml-auto lg:ml-0">
          {/* Phones reach the theme toggle via the Menu tab — the top bar hasn't room. */}
          <button onClick={toggle} className="nav-link !px-2.5 hidden sm:block" title={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'}
          ...
          <div className="relative" ref={menuRef}>
            <button onClick={() => setMenuOpen(o => !o)} aria-haspopup="menu" aria-expanded={menuOpen} aria-controls="account-menu" aria-label="Account menu" className="flex items-center gap-2 rounded-xl p-1 sm:pr-2 hover:bg-ink-800 transition-colors">
              <Avatar src={user.photo} name={user.name} size={8} />
              <svg className="w-3.5 h-3.5 text-mist-400 hidden sm:block" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
            </button>
            {menuOpen && (
